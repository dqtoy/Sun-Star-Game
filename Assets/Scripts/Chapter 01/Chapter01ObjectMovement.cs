...
public class Chapter01ObjectMovement : MonoBehaviour {
    ...
    {
        GameObject hh = null;
        if(GameManager.level1 <= 1)
            hh = Instantiate(help, new Vector3(0, 0, -2.1f), transform.rotation) as GameObject;
        ...
        {
            redBallClones[i].GetComponent<Rigidbody2D>().AddForce(new Vector2(40, 40), ForceMode2D.Impulse);
            redBallClones[i].transform.Rotate(new Vector3(0, 0, 1));
            blueBallClones[i].transform.Rotate(new Vector3(0, 0, 1));
            blueBallClones[i].GetComponent<Rigidbody2D>().AddForce(new Vector2(40, 40), ForceMode2D.Impulse);
    ...
    IEnumerator Delay()
    {
        ...
        switch (GameManager.level1)
        {
            case 1:
                ...
                if (PlayerPrefs.GetInt("chapter01_level") < GameManager.level1 + 1)
                {
                    PlayerPrefs.SetInt("chapter01_level", GameManager.level1 + 1);
                    PlayerPrefs.Save();
                }
                break;
            case 2:
        ...
        switch (GameManager.level1)
        {
            ...
            case 6:
                Application.LoadLevel("Chapter01Stage0" + (Random.Range(1,11)));
                break;
