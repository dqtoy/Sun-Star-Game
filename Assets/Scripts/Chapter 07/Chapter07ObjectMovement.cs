...
public class Chapter07ObjectMovement : MonoBehaviour {
    ...
    void Start()
    {
        ...
        blackBallClones = new GameObject[ballsCount];
        for (int i = 0; i < ballsCount; i++)
        {
           ...
           // ///redBallClones[i].GetComponent<Rigidbody2D>().AddForce(new Vector2(40, 40), ForceMode2D.Impulse);
            ...
            blackBallClones[i] = Instantiate(blackBallObject, new Vector3(Random.Range(5f,-5f), Random.Range(2f, -2f), -2), transform.rotation) as GameObject;
            //blackBallClones[i].GetComponent<Rigidbody2D>().AddForce(new Vector2(40, 40), ForceMode2D.Impulse);
    ...
    void Update()
    {
        ...
        if (START)
        {
            ...
            for (int i = 0; i < ballsCount; i++)
            {
                if (blackBallClones[i].transform.position.x > sliderLeft.transform.position.x && blackBallClones[i].transform.position.x < sliderRight.transform.position.x)
                {
                    black++;
                }
            }
            if (red == ballsCount && blue == ballsCount && black == ballsCount)
            {
                if (A)
                {
            ...
            for (int i = 0; i < ballsCount; i++)
            {
                ...
                ////
                if (Mathf.Abs(blackBallClones[i].GetComponent<Rigidbody2D>().velocity.x) < 4 && Mathf.Abs(blackBallClones[i].GetComponent<Rigidbody2D>().velocity.y) < 4)
                {
                    blackBallClones[i].GetComponent<Rigidbody2D>().velocity = new Vector3(blackBallClones[i].GetComponent<Rigidbody2D>().velocity.x * 1.1f, blackBallClones[i].GetComponent<Rigidbody2D>().velocity.y * 1.1f, 0);
                }
                if (Mathf.Abs(blackBallClones[i].GetComponent<Rigidbody2D>().velocity.x) > 8)
                {
                    blackBallClones[i].GetComponent<Rigidbody2D>().velocity = new Vector3(blackBallClones[i].GetComponent<Rigidbody2D>().velocity.x / 1.1f, 0, 0);
                }
                if (Mathf.Abs(blackBallClones[i].GetComponent<Rigidbody2D>().velocity.y) > 8)
                {
                    blackBallClones[i].GetComponent<Rigidbody2D>().velocity = new Vector3(0, blackBallClones[i].GetComponent<Rigidbody2D>().velocity.y / 1.1f, 0);
                }
    ...
    IEnumerator Delay()
    {
        ...
        switch (GameManager.level7)
        {
            case 1:
                ...
                if (PlayerPrefs.GetInt("chapter07_level") < GameManager.level7 + 1)
                {
                    PlayerPrefs.SetInt("chapter07_level", GameManager.level7 + 1);
                    PlayerPrefs.Save();
                } break;
            case 2:
        ...
        switch (GameManager.level7)
        {
            ...
            case 6:
                Application.LoadLevel("Chapter07Stage0" + (Random.Range(1, 11)));
                break;
