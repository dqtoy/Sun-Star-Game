    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            if (hit.collider != null)
            {
                ...
                if (hit.collider.name == "Level01" && GameManager.level3 >= 1)
                {
                    GameManager.level3 = 1;
                    GameManager.ballLsCount = 3;
                    Application.LoadLevel(selectLevel());
                }
                else if (hit.collider.name == "Level02" && GameManager.level3 >= 2)
                {
                    GameManager.level3 = 2;
                    GameManager.ballLsCount = 4;
                    Application.LoadLevel(selectLevel());
                }
                else if (hit.collider.name == "Level03" && GameManager.level3 >= 3)
                {
                    GameManager.level3 = 3;
                    GameManager.ballLsCount = 5;
                    Application.LoadLevel(selectLevel());
                }
                else if (hit.collider.name == "Level04" && GameManager.level3 >= 4)
                {
                    GameManager.level3 = 4;
                    GameManager.ballLsCount = 6;
                    Application.LoadLevel(selectLevel());
                }
                else if (hit.collider.name == "Level05" && GameManager.level3 >= 5)
                {
                    GameManager.level3 = 5;
                    GameManager.ballLsCount = 7;
                    Application.LoadLevel(selectLevel());
                }
                else if (hit.collider.name == "Level06" && GameManager.level3 >= 6)
                {
                    GameManager.level3 = 6;
                    GameManager.ballLsCount = 8;
                    Application.LoadLevel(selectLevel());
                }
                else if (hit.collider.name == "Level07" && GameManager.level3 >= 7)
                {
                    GameManager.level3 = 7;
